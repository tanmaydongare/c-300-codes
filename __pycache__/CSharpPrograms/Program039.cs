// Question: Program to calculate salary with allowances
...
public class Program039
{
    public static async Task Main()
    {
        ...
        if (!decimal.TryParse(Console.ReadLine(), out var basic) || basic < 0) { Console.WriteLine("Invalid input"); return; }
        Console.Write("Enter HRA (%): ");
        if (!decimal.TryParse(Console.ReadLine(), out var hraP) || hraP < 0) { Console.WriteLine("Invalid input"); return; }
        Console.Write("Enter DA (%): ");
        if (!decimal.TryParse(Console.ReadLine(), out var daP) || daP < 0) { Console.WriteLine("Invalid input"); return; }
        var hra = basic * hraP / 100m;
        var da = basic * daP / 100m;
