// Question: Program to demonstrate LINQ (Where, Select)
...
public class Program129
{
    public static async Task Main()
    {
        ...
        Console.WriteLine("Where+Select: " + string.Join(" ", evens));
        var grouped = nums.GroupBy(n => n % 2 == 0 ? "Even" : "Odd");
        foreach (var g in grouped) Console.WriteLine(g.Key + ": " + string.Join(" ", g));
        var people = new[] { (Id: 1, Name: "A"), (Id: 2, Name: "B") };
        var scores = new[] { (Id: 1, Score: 90), (Id: 2, Score: 80) };
        var joined = people.Join(scores, p => p.Id, s => s.Id, (p, s) => (p.Name, s.Score));
        Console.WriteLine("Join: " + string.Join(", ", joined.Select(x => x.Name + "=" + x.Score)));
        var agg = nums.Aggregate(0, (acc, n) => acc + n);
        Console.WriteLine($"Aggregate sum: {agg}");
    }
