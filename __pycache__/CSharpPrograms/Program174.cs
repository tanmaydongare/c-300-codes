// Question: REST API with Caching (Redis)
...
public class Program174
{
    ...
    {
        Console.WriteLine("Caching demo (in-memory cache with TTL)");
        var cache = new System.Collections.Generic.Dictionary<string, (string Value, DateTime Expiry)>();
        string Get(string key, Func<string> factory)
        {
            if (cache.TryGetValue(key, out var entry) && entry.Expiry > DateTime.UtcNow) return entry.Value;
            var v = factory();
            cache[key] = (v, DateTime.UtcNow.AddSeconds(5));
            return v;
        }
        for (int i = 0; i < 3; i++)
        {
            var v = Get("time", () => DateTime.UtcNow.ToString("O"));
            Console.WriteLine(v);
