// Question: Program to find missing number in array
...
public class Program095
{
    public static async Task Main()
    {
        ...
        if (arr.Length == 0) { Console.WriteLine("Empty array."); return; }
        int n = arr.Length + 1;
        long expected = (long)n * (n + 1) / 2;
        long actual = 0; foreach (var v in arr) actual += v;
        Console.WriteLine($"Missing number (assuming 1..{n} with one missing) = {expected - actual}");
    }
