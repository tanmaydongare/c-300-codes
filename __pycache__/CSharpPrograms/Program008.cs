// Question: Program to calculate compound interest
...
public class Program008
{
    public static async Task Main()
    {
        ...
        if (!double.TryParse(Console.ReadLine(), out var p)) { Console.WriteLine("Invalid input"); return; }
        Console.Write("Enter rate (% per annum): ");
        if (!double.TryParse(Console.ReadLine(), out var r)) { Console.WriteLine("Invalid input"); return; }
        Console.Write("Enter time (years): ");
        if (!double.TryParse(Console.ReadLine(), out var t)) { Console.WriteLine("Invalid input"); return; }
        Console.Write("Enter compounds per year: ");
        if (!int.TryParse(Console.ReadLine(), out var n) || n <= 0) { Console.WriteLine("Invalid input"); return; }
        var amount = p * Math.Pow(1 + (r / 100.0) / n, n * t);
        var ci = amount - p;
