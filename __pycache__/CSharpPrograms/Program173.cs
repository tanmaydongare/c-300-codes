// Question: REST API with Entity Framework Core
...
public class Program173
{
    ...
    {
        Console.WriteLine("This topic usually requires external dependencies (DB/EF Core).");
        Console.WriteLine("Console-friendly simulation: in-memory repository with CRUD.");
        var repo = new Repo();
        repo.Create("Alice");
        repo.Create("Bob");
        Console.WriteLine("All: " + string.Join(", ", repo.ReadAll().Select(x => x.Id + ":" + x.Name)));
        repo.Update(1, "Alice Updated");
        repo.Delete(2);
        Console.WriteLine("After changes: " + string.Join(", ", repo.ReadAll().Select(x => x.Id + ":" + x.Name)));
    }
