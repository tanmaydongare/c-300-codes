// Question: Program to demonstrate type casting
...
public class Program026
{
    public static async Task Main()
    {
        ...
        if (!double.TryParse(Console.ReadLine(), out var d)) { Console.WriteLine("Invalid input"); return; }
        int asInt = (int)d;
        long asLong = (long)d;
        Console.WriteLine($"double: {d}");
        Console.WriteLine($"(int)double: {asInt}");
        Console.WriteLine($"(long)double: {asLong}");
        Console.WriteLine($"Convert.ToString(double): {Convert.ToString(d)}");
    }
