// Question: Program to perform matrix multiplication
...
public class Program060
{
    public static async Task Main()
    {
        ...
        {
            Console.Write($"Enter rows for {name}: ");
            if (!int.TryParse(Console.ReadLine(), out var r) || r <= 0) return new int[0,0];
            Console.Write($"Enter cols for {name}: ");
            if (!int.TryParse(Console.ReadLine(), out var c) || c <= 0) return new int[0,0];
            var m = new int[r, c];
            Console.WriteLine($"Enter {r*c} elements row-wise for {name}:");
            for (int i = 0; i < r; i++)
                for (int j = 0; j < c; j++)
                {
                    if (!int.TryParse(Console.ReadLine(), out m[i, j])) { Console.WriteLine("Invalid"); return new int[0,0]; }
                }
            return m;
        }
        void Print(int[,] m)
        ...
        {
            Console.WriteLine("Matrix multiplication requires A.cols == B.rows.");
            return;
        }
        var r = a.GetLength(0);
        var k = a.GetLength(1);
        var c = b.GetLength(1);
        var prod = new int[r, c];
        for (int i = 0; i < r; i++)
            for (int j = 0; j < c; j++)
            {
                int s = 0;
