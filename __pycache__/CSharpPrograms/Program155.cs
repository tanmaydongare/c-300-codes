// Question: Console-based Inventory Management System
...
public class Program155
{
    ...
    {
        Console.WriteLine("Inventory Management System (in-memory, console-based)");
        Console.WriteLine("Commands: add, list, delete, exit");
        var items = new System.Collections.Generic.Dictionary<int, string>();
        int nextId = 1;
        while (true)
        {
            Console.Write("> ");
            ...
            if (cmd == "add")
            {
                ...
                items[nextId] = name;
                Console.WriteLine($"Added id={nextId}");
                nextId++;
            }
            else if (cmd == "list")
            {
                foreach (var kv in items) Console.WriteLine($"{kv.Key}: {kv.Value}");
                if (items.Count == 0) Console.WriteLine("(empty)");
            }
            else if (cmd == "delete")
            {
                Console.Write("Enter id: ");
                if (int.TryParse(Console.ReadLine(), out var id) && items.Remove(id)) Console.WriteLine("Deleted");
                else Console.WriteLine("Not found/invalid");
            }
            else
            {
