// Question: Program to generate random numbers
...
public class Program036
{
    public static async Task Main()
    {
        ...
        Console.Write("Min (inclusive): ");
        if (!int.TryParse(Console.ReadLine(), out var min)) { Console.WriteLine("Invalid input"); return; }
        Console.Write("Max (exclusive): ");
        if (!int.TryParse(Console.ReadLine(), out var max) || max <= min) { Console.WriteLine("Invalid input"); return; }
        var rng = new Random();
        for (int i = 0; i < n; i++) Console.WriteLine(rng.Next(min, max));
    }
