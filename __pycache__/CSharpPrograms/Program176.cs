// Question: REST API with Pagination and Filtering
...
public class Program176
{
    ...
    {
        Console.WriteLine("Console-hosted minimal REST server using HttpListener (no ASP.NET Core dependency).\n" +
                          "Run and browse: http://localhost:8085/hello\nPress Ctrl+C to stop.");
        ...
        while (true)
        {
            ...
            try
            {
                ...
                if (path == "/hello")
                {
                    var payload = System.Text.Json.JsonSerializer.Serialize(new { message = "Hello", time = DateTime.Now });
