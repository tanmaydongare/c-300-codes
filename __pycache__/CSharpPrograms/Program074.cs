// Question: Program to generate Fibonacci using recursion
...
public class Program074
{
    ...
    {
        long Fib(int n) => n switch { <= 0 => 0, 1 => 0, 2 => 1, _ => Fib(n - 1) + Fib(n - 2) };
        ...
        if (value <= 0) { Console.WriteLine("Terms must be positive."); return; }
        for (int i = 1; i <= value; i++) Console.Write(Fib(i) + (i == value ? "" : ", "));
        Console.WriteLine();
    }
