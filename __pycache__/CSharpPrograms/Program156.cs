// Question: File-based CRUD application
...
public class Program156
{
    ...
    {
        Console.WriteLine("File-based CRUD (JSON lines)");
        var path = "crud.jsonl";
        Console.WriteLine($"File: {path}");
        Console.WriteLine("Commands: create, read, update, delete, exit");
        var data = new System.Collections.Generic.Dictionary<int, string>();
        ...
        int nextId = data.Count == 0 ? 1 : data.Keys.Max() + 1;
        void Persist()
        {
            var lines = data.OrderBy(kv => kv.Key)
                .Select(kv => System.Text.Json.JsonSerializer.Serialize(new { id = kv.Key, name = kv.Value }))
                .ToArray();
        ...
        while (true)
        {
            ...
            if (cmd == "create")
            {
                ...
                data[nextId] = name;
                Console.WriteLine($"Created id={nextId}");
                nextId++;
                Persist();
            }
            else if (cmd == "read")
            {
                ...
                if (data.Count == 0) Console.WriteLine("(empty)");
            }
            else if (cmd == "update")
            {
                Console.Write("Id: ");
                if (!int.TryParse(Console.ReadLine(), out var id) || !data.ContainsKey(id)) { Console.WriteLine("Not found"); continue; }
            ...
            {
                Console.Write("Id: ");
                if (int.TryParse(Console.ReadLine(), out var id) && data.Remove(id)) { Persist(); Console.WriteLine("Deleted"); }
