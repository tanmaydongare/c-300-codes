// Question: REST API with Role-based Authorization
...
public class Program170
{
    ...
    {
        Console.WriteLine("JWT/Role-based REST requires a full web framework; console demo instead shows token-like flow.");
        var secret = "demo-secret";
        Console.Write("Username: ");
        var user = Console.ReadLine() ?? "";
        Console.Write("Role (Admin/User): ");
        var role = Console.ReadLine() ?? "User";
        var token = Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(user + ":" + role + ":" + secret));
        Console.WriteLine($"Issued token: {token}");
        Console.Write("Enter token to validate: ");
        var given = Console.ReadLine() ?? "";
        try
        {
            var raw = System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(given));
            var parts = raw.Split(':');
            if (parts.Length != 3 || parts[2] != secret) throw new Exception("Invalid token");
            Console.WriteLine($"Token OK. user={parts[0]}, role={parts[1]}");
            Console.WriteLine(parts[1] == "Admin" ? "Authorized" : "Forbidden for admin action");
        }
        catch { Console.WriteLine("Invalid token"); }
    }
