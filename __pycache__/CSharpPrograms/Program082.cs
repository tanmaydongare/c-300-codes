// Question: Program to find second largest element in array
    ...
    public static async Task Main()
    {
        int[] ReadIntArray()
        {
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                ...
                {
                    Console.WriteLine("Invalid integer in array.");
        ...
        if (arr.Length < 2) { Console.WriteLine("Need at least 2 elements."); return; }
        Array.Sort(arr);
        Array.Reverse(arr);
        int first = arr[0];
        int? second = null;
        ...
        {
            if (v != first) { second = v; break; }
        }
        Console.WriteLine(second.HasValue ? $"Second largest = {second.Value}" : "No distinct second largest.");
    }
