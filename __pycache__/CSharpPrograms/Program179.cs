// Question: Message Queue using RabbitMQ (Basic)
...
public class Program179
{
    ...
    {
        Console.WriteLine("Message queue demo (in-memory producer/consumer)");
        var q = new System.Collections.Concurrent.BlockingCollection<string>();
        var consumer = System.Threading.Tasks.Task.Run(() =>
        {
            foreach (var msg in q.GetConsumingEnumerable()) Console.WriteLine("Consumed: " + msg);
        });
        for (int i = 1; i <= 5; i++)
        {
            q.Add("msg-" + i);
