// Question: Role-based Authentication System
...
public class Program160
{
    ...
    {
        Console.WriteLine("Role-based Authentication (in-memory)");
        var users = new System.Collections.Generic.Dictionary<string, (string Password, string Role)>
        {
        ...
        if (!users.TryGetValue(u, out var info) || info.Password != p)
        {
            Console.WriteLine("Login failed");
            return;
        }
        Console.WriteLine($"Login OK. Role={info.Role}");
        Console.WriteLine(info.Role == "Admin" ? "Access granted to admin feature" : "Admin feature denied");
    }
