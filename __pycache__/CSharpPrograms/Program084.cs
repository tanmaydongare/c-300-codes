// Question: Program to check anagram strings
...
public class Program084
{
    public static async Task Main()
    {
        ...
        string Normalize(string s) => new string(s.Where(char.IsLetterOrDigit).Select(char.ToLowerInvariant).OrderBy(c => c).ToArray());
        Console.WriteLine(Normalize(a) == Normalize(b) ? "Anagrams" : "Not anagrams");
    }
